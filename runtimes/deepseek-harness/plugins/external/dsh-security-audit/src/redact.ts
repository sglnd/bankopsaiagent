 ...
 */
export const TEST_TOKEN_ALLOWLIST: readonly RegExp[] = [
...
interface SecretPattern {
  kind: SecretKind
  re: RegExp
  /** generic 模式使用捕获组 1 作为候选值；否则整段匹配。 */
  group?: 1
}
...
function isAllowlisted(value: string, allowlist: readonly RegExp[]): boolean {
...
export function scanSecrets(text: string, allowlist: readonly RegExp[] = TEST_TOKEN_ALLOWLIST): SecretHit[] {
...
export function classifySecret(value: string): { kind: SecretKind; length: number } | null {
  for (const { kind, re } of SECRET_PATTERNS) {
    if (re.test(value)) return { kind, length: value.length }
  }
  return null
}
 ...
 * 输出保留原始大小写。
 */
export function redactPath(p: string, root: string, home: string, win: boolean = process.platform === 'win32'): string {
  const normalize = (s: string): string => s.replace(/\\/g, '/').replace(/\/+/g, '/').replace(/\/$/, '')
  const orig = normalize(p)
  const cmp = (s: string): string => (win ? normalize(s).toLowerCase() : normalize(s))
  const r = cmp(root)
  const h = cmp(home)
  const t = cmp(orig)
...
export function redactPathInText(text: string, root: string, home: string): string {
  let out = text
  const replace = (abs: string, token: string): void => {
    if (abs === '') return
...
export function displayUrl(raw: string): string {
  try {
    ...
    let path = u.pathname
    if (path.length > 24) path = path.slice(0, 12) + '…' + path.slice(-8)
...
export function safeErrorMessage(error: unknown, root: string, home: string): string {
