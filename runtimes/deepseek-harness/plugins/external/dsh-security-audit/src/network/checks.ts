...
import { readFileCapped, throwIfAborted, throwIfDeadlineExceeded } from '../paths.ts'
import { displayUrl, redactPath } from '../redact.ts'
import { RULE_BY_CODE } from '../rules.ts'
import type { AuditContext, CheckResult, Finding, ScannerResult } from '../types.ts'
import { discoverConfigFiles, type ConfigFile } from '../config/discover.ts'
import { parseSafeByKind, scalarValues } from '../config/parse-safe.ts'
import { classifyHostname, classifyUrl, isAllowedEndpoint } from './classify.ts'
...
function pushFinding(_ctx: AuditContext, code: string, subject: string, t: {
  severity: import('../types.ts').Severity
  exposure: string
  recommendation: string
  confidence: import('../types.ts').Confidence
  evidence?: import('../types.ts').Evidence
}): Finding {
...
interface NetworkState {
  ...
  /** 认证证据必须与监听声明同文件（env 中的客户端 key 不豁免服务端监听）。 */
  listenAuth: boolean
  ...
  proxySubject?: string
  discoveryTargets: Array<{ subject: string; value: string }>
  httpTargets: Array<{ subject: string; value: string }>
  listened: boolean
}

async function scanFile(ctx: AuditContext, file: ConfigFile, state: NetworkState): Promise<void> {
  ...
  if (isCredentials) state.hasCredentials = true

  let fileHasAuth = false
  let fileListen: { host: string; subject: string } | undefined
  ...
  for (const [k, v] of iterable) {
    ...
    if (AUTH_KEY_RE.test(k)) {
      if (!/^(true|false|null)$/i.test(v) && !isCredentials) fileHasAuth = true
    }
...
export async function scanNetwork(ctx: AuditContext): Promise<ScannerResult> {
  ...
  for (const t of state.httpTargets) {
    if (httpSeen.has(t.value)) continue
  ...
  for (const t of state.discoveryTargets) {
    if (discSeen.has(t.value)) continue
