 ...
 */
export function classifyHostname(host) {
    const h = host.trim().toLowerCase().replace(/^\[|\]$/g, '');
    if (h === '' || h === 'localhost' || h === 'localhost.localdomain' || h === '::1')
        return 'loopback';
    if (h === '0.0.0.0' || h === '::' || h === '[*]')
        return 'unspecified';
    if (/^127\./.test(h))
    ...
    if (/^172\.(1[6-9]|2\d|3[01])\./.test(h))
        return 'private';
    if (/^fc|^fd/.test(h))
        return 'private'; // fc00::/7 (IPv6 unique local)
    if (/^fe[89ab]/.test(h))
        return 'private'; // fe80::/10 link-local
    if (/^[0-9a-f:]+$/.test(h))
        return 'unknown'; // 其它 IPv6 字面量
    return 'external';
}
 ...
 */
export function isAllowedEndpoint(raw, allowlist) {
    ...
    return allowlist.some((entry) => {
        ...
        if (isInvalidAllowlistEntry(entry))
            return false;
        const e = normalizeEndpoint(entry);
        return e !== null && e === norm;
    });
}
function isInvalidAllowlistEntry(entry) {
    let url;
    try {
        url = new URL(entry);
    }
