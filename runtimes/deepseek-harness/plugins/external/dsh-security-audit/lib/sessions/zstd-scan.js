...
function decodeFcs(dv, offset, sizeBytes) {
    if (sizeBytes === 1)
        return dv.getUint8(offset);
    if (sizeBytes === 2)
        return 256 + dv.getUint16(offset, true);
    if (sizeBytes === 4)
        return 65536 + dv.getUint32(offset, true);
...
export function parseFrameHeader(buf) {
    ...
    if (contentSizeBytes > 0) {
        if (p + contentSizeBytes > buf.byteLength)
            return null;
 ...
 */
export function analyzeZstd(buf, compressedSize) {
    const base = { isZstd: false, headerComplete: false, torn: false, compressedSize, frames: 0 };
    if (buf.byteLength < 4) {
...
export function isTempResidue(name) {
