...
import { isValidPackageName, isValidSemver, isValidSemverRange, resolveWithin } from './paths.ts'
import type { CheckIssue } from './report.ts'

const REGISTRY_ID_RE = /^[a-z0-9][a-z0-9-]*(\/[a-z0-9][a-z0-9-]*)?$/

export async function checkRegistry(dir: string): Promise<CheckIssue[]> {
  ...
  if (typeof main !== 'string' || main === '') {
    ...
    if (!r.ok) issues.push({ code: 'registry-main-missing', detail: `main 无效: ${r.reason}` })
  }

  // client.main：同 containment（若声明 client）
  const client = manifest['client'] as Record<string, unknown> | undefined
  if (client !== undefined && typeof client === 'object') {
    if (typeof client['main'] === 'string' && client['main'] !== '') {
      const r = await resolveWithin(dir, client['main'] as string)
      if (!r.ok) issues.push({ code: 'registry-client-main', detail: `client.main 无效: ${r.reason}` })
    }
