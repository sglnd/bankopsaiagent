...
async function hasSkillsDir(dir: string): Promise<boolean> {
  try {
    ...
    for (const e of entries) {
      if (await exists(join(dir, 'skills', e, 'SKILL.md'))) return true
    }
  } catch { /* 无 skills 目录 */ }
  return false
}
...
export function looksLikeToolPlugin(srcTexts: string[]): boolean {
  return srcTexts.some(t =>
    /(?:from\s+|import\s*\(|require\s*\(|import\s+)['"]@deepseek-ai\/dsh-tools(?:\/[a-zA-Z0-9._-]+)?['"]/.test(t),
  )
}

/** 检测目录的项目形态。 */
export async function detectKind(dir: string): Promise<RepoKind> {
  ...
  if (hasCatalog) {
    ...
    try {
      ...
      if (typeof parsed['collection'] === 'string' || Array.isArray(parsed['plugins'])) return 'collection'
    } catch { /* 非 collection */ }
  }
  if (hasPkg) {
...
export async function collectSrcTexts(dir: string, limitBytes = 512 * 1024): Promise<string[]> {
  const out: string[] = []
