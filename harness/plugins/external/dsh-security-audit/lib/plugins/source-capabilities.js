...
export async function scanSourceCapabilities(pkgDir, opts) {
    const hits = [];
    const warnings = [];
    const seen = new Set();
    const visitFile = async (file) => {
        ...
        if (text.length > LIMITS.sourceFileBytes)
            return;
 ...
 */
export function capabilityFindings(ctx, pkgName, hits) {
    const findings = [];
    const exposures = {
...
export async function findSecretLikeFiles(pkgDir, opts) {
    const found = [];
    const walk = async (dir, depth) => {
        throwIfAborted(opts.signal);
        if (depth > 2)
