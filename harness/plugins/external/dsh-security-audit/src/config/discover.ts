...
function classifyName(name: string): ConfigFileKind {
  if (name === '.env' || name === 'env') return 'env'
  if (SETTINGS_NAMES.includes(name.toLowerCase())) return 'settings'
  if (CREDENTIALS_RE.test(name)) return 'credentials'
  if (CONFIG_NAMES.includes(name.toLowerCase())) return 'config'
  if (name === 'package.json') return 'package'
  if (name === 'cordis.patch.yml' || name === 'cordis.yml') return 'patch'
  if (SECRETISH_RE.test(name)) return 'credentials'
  return 'other'
}
...
async function collectDir(
  ...
  for (const entry of entries) {
    ...
    if (stat.isSymbolicLink()) continue // 拒绝 symlink/reparse escape
...
export async function discoverConfigFiles(
  root: string,
  opts: { profile?: string; signal?: AbortSignal; deadline: number; maxFiles?: number } = { deadline: Number.POSITIVE_INFINITY },
  ...
  if (profilesStat !== null && profilesStat.isDirectory()) {
    ...
    for (const p of entries) {
      ...
      if (opts.profile !== undefined && p.name !== opts.profile) continue
      const pStat = await lstatSafe(path.join(profilesDir, p.name))
      if (pStat === null || !pStat.isDirectory() || pStat.isSymbolicLink()) continue
