...
import { displayUrl, redactPath, safeErrorMessage, scanSecrets } from '../redact.ts'
import { RULE_BY_CODE } from '../rules.ts'
import type { AuditContext, CheckResult, Evidence, Finding, ScannerResult } from '../types.ts'
import { discoverConfigFiles, type ConfigFile } from './discover.ts'
import { parseSafeByKind, scalarValues } from './parse-safe.ts'
import { checkFilePermissions } from '../platform/permissions.ts'
...
function makeFinding(_ctx: AuditContext, code: string, subject: string, t: FindingTemplate): Finding {
...
function checkSecrets(ctx: AuditContext, file: ConfigFile, text: string, checks: CheckResult[], findings: Finding[]): void {
  const rel = file.rel
...
function checkPathFields(ctx: AuditContext, file: ConfigFile, data: Record<string, unknown>, checks: CheckResult[], findings: Finding[]): void {
  let flagged = false
  for (const [k, v] of scalarValues(data)) {
    if (typeof v !== 'string') continue
    if (!/(^|\.)(link|patch|root|path)$/i.test(k) && !/^link$|^patch$|^root$/i.test(k)) continue
    if (/^[a-z][a-z0-9+.-]*:/i.test(v) || v.startsWith('/')) continue // URL 或绝对路径另行处理
    if (v.startsWith('.')) {
...
function checkCredentialTargets(ctx: AuditContext, file: ConfigFile, data: Record<string, unknown>, text: string, checks: CheckResult[], findings: Finding[]): void {
  const redactedPath = redactPath(file.path, ctx.root, ctx.home)
  const subjects: Array<{ key: string; value: string }> = []
  for (const [k, v] of scalarValues(data)) {
    if (typeof v !== 'string') continue
    if (!CREDENTIAL_TARGET_KEY_RE.test(k) && !URL_KEY_RE.test(k)) continue
...
function checkEnvExpansions(ctx: AuditContext, text: string, rel: string, checks: CheckResult[], findings: Finding[]): void {
  const missing = new Set<string>()
  for (const m of text.matchAll(ENV_PLACEHOLDER_RE)) {
    const name = m[1]!
    if (!(name in ctx.env)) missing.add(name)
  }
...
async function checkCredentialsPermissions(ctx: AuditContext, file: ConfigFile, checks: CheckResult[]): Promise<void> {
...
export async function scanConfig(ctx: AuditContext): Promise<ScannerResult> {
  ...
  for (const file of discovery.files) {
    ...
    if (file.kind === 'package' || file.kind === 'patch') continue
