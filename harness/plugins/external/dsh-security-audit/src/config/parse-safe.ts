...
export function parseJsonSafe(text: string): SafeParseResult {
  try {
    const value: unknown = JSON.parse(text)
    if (value !== null && typeof value === 'object' && !Array.isArray(value)) {
      return { ok: true, data: flattenObject(value as Record<string, unknown>, '') }
    }
    return { ok: true, data: {} }
  } catch {
...
export function parseEnvSafe(text: string): SafeParseResult {
  const data: Record<string, unknown> = {}
  for (const raw of text.split(/\r?\n/)) {
    const line = raw.replace(/^\uFEFF/, '').trim()
    if (line === '' || line.startsWith('#')) continue
    const eq = line.indexOf('=')
    if (eq <= 0) return { ok: false, reason: 'unsupported-env-line' }
    const key = line.slice(0, eq).trim()
    if (!KEY_RE.test(key)) return { ok: false, reason: `unsupported-key:${key}` }
    data[key] = unquote(line.slice(eq + 1).trim())
  }
  return { ok: true, data }
}

function flattenObject(obj: Record<string, unknown>, prefix: string, out: Record<string, unknown> = {}): Record<string, unknown> {
 ...
 */
export function parseYamlSafe(text: string): SafeParseResult {
  ...
  const parentPath = (): string => stack.map((s) => s.key).join('.')

  for (const { indent, text } of lines) {
    if (text.startsWith('- ')) {
      const rest = text.slice(2).trim()
      if (listPath === null || indent <= listIndent) {
        if (stack.length === 0) return { ok: false, reason: 'top-level-list-unsupported' }
        const parentKey = parentPath()
        const existing = out[parentKey]
        if (existing !== undefined && !Array.isArray(existing)) {
          // 容器占位（{}）→ 转为列表；非占位冲突则拒绝
          if (typeof existing === 'object' && existing !== null && Object.keys(existing as object).length === 0) {
            out[parentKey] = []
          } else {
      ...
      const arr = out[listPath] as unknown[]
    ...
    if (colon <= 0) return { ok: false, reason: 'unsupported-line' }
  ...
  for (const [k, v] of Object.entries(out)) {
    if (v !== null && typeof v === 'object' && !Array.isArray(v) && Object.keys(v as object).length === 0 && k !== '') {
      continue
    }
    final[k] = v
  }
...
export function parseSafeByKind(text: string, kind: ConfigFileKind): SafeParseResult {
  if (kind === 'env') return parseEnvSafe(text)
  if (text.trimStart().startsWith('{')) return parseJsonSafe(text)
  return parseYamlSafe(text)
}

/** 从扁平数据中按 dotted key 取路径值。 */
export function getPath(data: Record<string, unknown>, dotted: string): unknown {
  return data[dotted]
}

/** 遍历扁平数据所有标量值。 */
export function* scalarValues(data: Record<string, unknown>): Generator<[string, unknown]> {
  for (const [k, v] of Object.entries(data)) {
    if (Array.isArray(v)) {
      for (let i = 0; i < v.length; i++) yield [`${k}[${i}]`, v[i]]
