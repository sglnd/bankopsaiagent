...
import type { AuditCategory, RuleMeta, Severity } from './types.ts'
...
export const RULES: readonly RuleMeta[] = spec.map((r) => ({
  code: r.code,
  category: r.category,
  severity: r.severity,
...
export const RULE_BY_CODE: ReadonlyMap<string, RuleMeta> = new Map(RULES.map((r) => [r.code, r]))
