...
import { Redactor, redactPath, safeErrorMessage } from './redact.ts'
import { RULE_BY_CODE, RULES, SEVERITY_RANK } from './rules.ts'
import { scanConfig } from './config/checks.ts'
import { scanPlugins } from './plugins/discover.ts'
import { scanSessions } from './sessions/checks.ts'
import { scanNetwork } from './network/checks.ts'
import type {
  AuditContext,
  AuditParams,
  AuditReport,
...
const ACTIONS: readonly AuditParams['action'][] = ['scan_config', 'scan_plugins', 'scan_sessions', 'scan_network', 'report', 'rules']

export class AuditArgsError extends Error {
  override readonly name = 'AuditArgsError'
}

export async function buildRulesOutput(platform: NodeJS.Platform = process.platform): Promise<RulesOutput> {
  const rules: RuleCatalogEntry[] = RULES.map((r) => ({
    code: r.code,
    category: r.category,
    severity: r.severity,
...
async function buildContext(params: AuditParams, opts: RunOptionsLike): Promise<AuditContext> {
  ...
  for (const r of allowedRootsRaw) {
    ...
    if (real !== null) allowedRoots.push(real)
  }

  let root = fixedRoot
  if (params.root !== undefined && params.root !== '') {
    const real = await realpathSafe(params.root)
    if (real === null) throw new AuditArgsError('security_audit: provided root does not exist or cannot be resolved')
  ...
  const action = (params.action === 'rules' ? 'scan_config' : params.action) as Exclude<AuditParams['action'], 'rules'>
...
export function computeRiskVerdict(findings: readonly Finding[], strict: boolean): RiskVerdict {
  if (findings.some((f) => f.severity === 'critical' || f.severity === 'high')) return 'fail'
  if (findings.some((f) => f.severity === 'medium')) return strict ? 'fail' : 'warning'
  if (findings.some((f) => f.severity === 'low')) return 'warning'
  return 'pass'
}

const COVERAGE_SKIP_REASONS: ReadonlySet<SkipReason> = new Set(['platform', 'permission'])

export function computeCoverageVerdict(checks: readonly CheckResult[]): CoverageVerdict {
  for (const c of checks) {
    // 扫描器自身失败（internal-error）视为关键覆盖不全
    if (c.code === 'internal-error' && c.state === 'error') return 'incomplete'
    const rule = RULE_BY_CODE.get(c.code)
    if (rule === undefined || !rule.critical) continue
    if (c.state === 'error') return 'incomplete'
...
export function computeVerdict(risk: RiskVerdict, coverage: CoverageVerdict): Verdict {
  if (risk === 'fail') return 'fail'
  if (coverage === 'incomplete') return 'incomplete'
  if (risk === 'warning') return 'warning'
  return 'pass'
}

export function summarize(findings: readonly Finding[], checks: readonly CheckResult[]): ReportSummary {
  const summary: ReportSummary = { critical: 0, high: 0, medium: 0, low: 0, info: 0, passed: 0, skipped: 0, errors: 0 }
  for (const f of findings) {
    summary[f.severity]++
  }
  for (const c of checks) {
    if (c.state === 'pass') summary.passed++
    else if (c.state === 'skipped') summary.skipped++
    else if (c.state === 'error') summary.errors++
  }
  return summary
}

const SEVERITY_ORDER: Record<Severity, number> = SEVERITY_RANK
...
const STATE_ORDER: Record<CheckResult['state'], number> = { finding: 0, error: 1, skipped: 2, pass: 3 }

export function sortChecks(checks: readonly CheckResult[]): CheckResult[] {
...
export function buildReport(ctx: AuditContext, results: ScannerResult[], truncated: boolean): AuditReport {
...
async function runScanner(ctx: AuditContext, fn: (c: AuditContext) => Promise<ScannerResult>): Promise<ScannerResult> {
  try {
    return await fn(ctx)
  } catch (error) {
    if (error instanceof AuditAbortedError || error instanceof AuditTimeoutError) throw error
...
export async function runAction(params: AuditParams, opts: RunOptions = {}): Promise<AuditReport | RulesOutput> {
  ...
  const run = (fn: (c: AuditContext) => Promise<ScannerResult>): Promise<ScannerResult> => runScanner(ctx, fn)

  let results: ScannerResult[]
  if (params.action === 'report') {
    // 顺序执行四个子扫描器，每个之间检查 deadline/signal
    const parts: ScannerResult[] = []
    ...
    const fn =
      params.action === 'scan_config' ? scanConfig :
