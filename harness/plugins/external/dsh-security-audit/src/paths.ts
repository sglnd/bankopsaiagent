...
export function throwIfAborted(signal: AbortSignal | undefined): void {
  if (signal?.aborted) throw new AuditAbortedError('security_audit: cancelled')
}

export function throwIfDeadlineExceeded(deadline: number, signal: AbortSignal | undefined): void {
  throwIfAborted(signal)
  if (Date.now() > deadline) throw new AuditTimeoutError('security_audit: action deadline exceeded')
}

/** 字符串级 containment（不 realpath）；win32 大小写不敏感。 */
export function isWithinPath(parent: string, child: string, win: boolean = process.platform === 'win32'): boolean {
...
export async function lstatSafe(p: string): Promise<Stats | null> {
  try {
 ...
 */
export async function resolveContained(base: string, candidate: string, signal?: AbortSignal): Promise<ResolvedPath> {
  throwIfAborted(signal)
  const stat = await lstatSafe(candidate)
  if (stat === null) throw new PathNotFoundError(candidate)
...
export function assertWithin(base: string, candidate: string, win?: boolean): void {
 ...
 * 只读，绝不修改文件。
 */
export async function readFileCapped(p: string, maxBytes: number, signal?: AbortSignal): Promise<ReadResult> {
  throwIfAborted(signal)
...
export async function readHeadCapped(p: string, maxBytes: number, signal?: AbortSignal): Promise<ReadResult> {
  throwIfAborted(signal)
  const stat = await lstatSafe(p)
  if (stat === null) return { kind: 'missing' }
  if (!stat.isFile()) return { kind: 'error', message: 'not a regular file' }
