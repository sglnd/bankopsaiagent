...
function defaultFlowPart(readme: string): string {
...
export async function checkProfileInstallDocs(dir: string, kind: RepoKind): Promise<CheckIssue[]> {
  ...
  try {
    ...
    for (const e of entries) {
      if (!/\.(sh|ps1|mjs|js)$/.test(e)) continue
      try { scripts += await fs.readFile(join(dir, 'scripts', e), 'utf8') } catch { /* 跳过 */ }
    }
  } catch { /* 无 scripts 目录 */ }
...
export function isBundleInstallable(patchDeclared: boolean, docsIssues: CheckIssue[]): boolean {
...
export function checkCoreRowIds(entries: Array<{ id: string }>): CheckIssue[] {
  const issues: CheckIssue[] = []
  for (const e of entries) {
