...
import { collectTextsBounded } from './paths.js';
...
export async function resolveTsconfig(dir) {
    const read = async (filePath, depth) => {
        if (depth > EXTENDS_MAX_DEPTH)
            return { opts: {}, ok: false, reason: `extends 深度超过 ${EXTENDS_MAX_DEPTH}` };
        let raw;
        try {
        ...
        Object.assign(opts, (cfg['compilerOptions'] ?? {}));
        return { opts, ok: true };
    };
    try {
...
function usesBufferOrNode(texts) {
...
export async function checkBuildPitfalls(dir, pkg) {
    const issues = [];
    ...
    if (tsconfig !== null && tsconfig.resolved) {
        ...
        const hasExplicitNodeTypes = Array.isArray(opts['types']) && opts['types'].includes('node');
    ...
    if (!hasBuildPath || !hasPrepack) {
        if (!libEntryExists && !hasBuildPath) {
            ...
            if (!hasBuild && !hasPrepare)
                issues.push({ code: 'no-build-script', detail: 'package.json 缺 scripts.build（或 Git 安装型补 scripts.prepare）' });
            if (!hasPrepack)
