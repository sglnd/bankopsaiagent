...
Copy the project directory and the exported TAR to the Linux target, run
`scripts/import.sh`, then `docker compose up -d --no-build`. Image export makes
...
endpoint is responding. `scripts/smoke-test.sh` performs the stronger external
check: it sends the MCP `initialize` request to Playwright, Filesystem, and Git
and requires a successful JSON-RPC result. Run it after `docker compose up -d`
